...
## *2.*  Thiết kế bảng
Một cách đơn giản, chúng ta dùng [Bootstrap](http://getbootstrap.com/) để đánh bóng bảng của mình. Trong tệp tin `app/ideas/index.html.erb`, tìm dòng này:
{% highlight erb %}
<table>
{% endhighlight %}
và thay thế với bởi dòng:
{% highlight erb %}
<table class="table">
{% endhighlight %}
Chỉnh sửa kích thước của ảnh sử dụng dòng bên dưới:
{% highlight erb %}
...
Truy cập vào đường dẫn [http://localhost:3000/ideas/new](http://localhost:3000/ideas/new) và tìm nút `Create Idea`. Thêm đoạn code sau vào cuối file `app/assets/stylesheets/ideas.scss`:
